...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {
                ...
                /**
                 * 选择消息发送的队列
                 * 普通消息：默认方式下，selectOneMessageQueue从topicPublishInfo中的
                 * messageQueueList中选择一个队列（MessageQueue）进行发送消息，
                 * 默认采用长轮询的方式选择队列 。它的机制如下：正常情况下，
                 * 顺序选择queue进行发送；如果某一个节点发生了超时，则下次选择queue时，
                 * 跳过相同的broker。不同的队列选择策略形成了生产消息的几种模式，
                 * 如顺序消息，事务消息。
                 * 
                 */
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
             ...
             * 从nameserver端拉取最新路由信息大致为：首先getTopicRouteInfoFromNameServer，
             * 然后topicRouteData2TopicPublishInfo。
             *
             *
             */
